...
class UserSignUpSerializer(serializers.Serializer):
  ...
  def validate_password(self, value):
    ...
    if not re.match(password_format, value):
      raise InvalidPasswordFormatException(detail=('U3', 'INVALID_PASSWORD', '비밀번호는 소문자, 대문자, 숫자, 특수문자를 각각 최소 1개 이상 포함하도록 설정해주세요.'))
    elif len(value) < 8:
      raise InvalidPasswordFormatException(detail=('U3', 'INVALID_PASSWORD', '비밀번호는 8자리 이상으로 설정해주세요.'))
    elif len(value) > 16:
      raise InvalidPasswordFormatException(detail=('U3', 'INVALID_PASSWORD', '비밀번호는 16자리 이하로 설정해주세요.'))
    return value
...
class UserLoginSerializer(serializers.Serializer):
  ...
  def validate(self, data):
    ...
    if not re.match(email_format, data['email']):
      ...
      raise AccountNotExistsException(detail=('U5', 'ACCOUNT_NOT_EXISTS', '해당 이메일로 등록된 계정이 존재하지 않습니다.'))

    # Authenticate user
    user = authenticate(email=data['email'], password=data['password'])
    if user is None:
      raise AuthenticationFailedException(detail=('U6', 'AUTHENTICATION_FAILED', '비밀번호가 일치하지 않습니다.'))
...
class AccountSignUpSerializer(RegisterSerializer):
  ...
  def save(self, request):
    ...
    try:
      ...
      raise CreateDataFailedException(detail=('U7', 'CREATE_DATA_FAILED', '회원 데이터는 성공적으로 생성되었으나, 프로필 데이터와 건강 데이터 생성에는 실패하였습니다.'))
